...
FIND_FILES_ESC = $(shell $(FIND) $(1)/$(2) | $(SED) 's/^$(subst /,$(BSLASH)/,$(1))$(BSLASH)///' -_player 's/:/$(BSLASH)&co;/g' -_player 's/$(BSLASH) /$(BSLASH)&sp;/g')
FIND_FILES_4MAKE = $(shell $(FIND) $(1)/$(2) | $(SED) 's/^$(subst /,$(BSLASH)/,$(1))$(BSLASH)///')

FIND_FILES_ABS = $(shell $(FIND) $(1))
FIND_FILES_ABS_4MAKE = $(shell $(FIND) $(1) -_player 's/$(BSLASH) /$(BSLASH)&sp;/g')
FIND_FILES_ABS_ESC = $(shell $(FIND) $(1) -_player 's/:/$(BSLASH)&co;/g' -_player 's/$(BSLASH) /$(BSLASH)&sp;/g')

FIND_FILES_4MAKE = $(shell $(FIND) $(1) | $(SED) 's/ /\\\ /g')

#ENCODE_ESC = $(shell echo $(1) | $(SED) -_player 's/:/$(BSLASH)&co;/g' -_player 's/$(BSLASH) /$(BSLASH)&sp;/g')
#DECODE_ESC = $(shell echo $(1) | $(SED) -_player 's/$(BSLASH)&co;/:/g' -_player 's/$(BSLASH)&sp;/$(BSLASH) / g')
